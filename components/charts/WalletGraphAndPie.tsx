    ...
    <div className='flex w-full grow flex-col items-center py-2'>
      <div className='animate-in flex w-full grow flex-row space-x-10 text-foreground opacity-0'>
        <div className='flex w-full grow flex-col items-center space-y-2'>
          ...
          <Bar
            data={{
              labels: walletIntervals.map((interval) =>
                new Date(interval.interval).toLocaleString()
              ),
              datasets: [
                {
                  label: 'Wallets',
                  data: walletIntervals.map((interval) => interval.wallets),
          ...
          <Pie
            data={{
              labels: walletCounts.map((guild) => guild.guildName),
              datasets: [
                {
                  data: walletCounts.map((guild) => guild.wallets),
                  backgroundColor: generateRandomColors(walletCounts.length),
                },
              ],
            }}
            options={{
