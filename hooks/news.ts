...
type NewsInteractions = {
  ...
  choice:
    | Database['public']['Tables']['poll_choices']['Row']
    | Database['public']['Tables']['quiz_choices']['Row']
    | null;
  guildName: string;
  ...
  useEffect(() => {
    const fetchInteractions = async () => {
      const { data, error } = await supabase
        .from('interactions')
        .select(
          '*, discord_users(*), quiz_choices(*), poll_choices(*), guilds(name)'
        )
        .eq('news_id', newsId)
        .order('created_at', { ascending: false })
        .range(from, to);
    ...
    const fetchGuilds = async () => {
      if (search) {
        ...
        if (data) {
          setGuilds(
            await Promise.all(
              data.map(async (guilds) => {
                return {
                  ...guilds,
                  interactions: await getGuildInteractionsByNews(
                    supabase,
                    newsId,
                    guilds.id
                  ),
...
export async function getGuildViewsByNews(
  supabase: SupabaseClient<Database>,
  newsId: number,
  guildId: string
): Promise<number> {
...
export async function getGuildInteractionsByNews(
  supabase: SupabaseClient<Database>,
  newsId: number,
  guildId: string
): Promise<number> {
